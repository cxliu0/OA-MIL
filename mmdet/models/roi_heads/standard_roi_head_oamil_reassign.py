...
class StandardRoIHeadOAMIL(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        def update_gt_boxes(gt_bboxes, noisy_gt_boxes, pseudo_gt_boxes):
            ...
            for idx, img_gt_boxes in enumerate(gt_bboxes):
                for ii, img_gt_box in enumerate(img_gt_boxes):
                    ...
                    if len(m_pseudo_gt_box) > 0:
                        new_gt_bboxes[idx][ii] = m_pseudo_gt_box
            return new_gt_bboxes
        ...
        '''
        losses = dict()
        loss_oamil, loss_oaie = 0, 0
        
        # * The number of iteration is depend on whether to perform Object-Aware Instance Extension (OA-IE)
        #   - iter num=1 if w/o OA-IE 
        #   - iter num=N+1 if with OA-IE, where N is the number of OA-IE
        total_iter_num = self.bbox_head.oaie_num+1 if self.bbox_head.oaie_flag and self._epoch+1 >= self.bbox_head.oaie_epoch else 1
        for iter_num in range(total_iter_num):

            # update gt with best selected instances (b_i^* in the paper) when performing OA-IE
            if iter_num > 0:
            ...
            # apply OA-MIL
            loss_oamil, pseudo_bbox_targets, noisy_gt_boxes, pseudo_gt_boxes = self._oamil(bbox_targets, bbox_results, rois, x)
            ...
            if iter_num == 0:
                ...
                # compute OA-IE loss
                loss_oaie += loss_oamil['loss_oais']
            
            # update noisy gt for OA-IE
            new_gt_bboxes = update_gt_boxes(gt_bboxes, noisy_gt_boxes, pseudo_gt_boxes)
        
        # add OA-IE loss
        if self.bbox_head.oaie_flag and (self._epoch+1 >= self.bbox_head.oaie_epoch):            
            #losses['loss_oais'] = (losses['loss_oais'] + tmp_oamil)/ (self.bbox_head.oaie_num + 1)
            losses['loss_oais'] = losses['loss_oais'] + (loss_oaie/self.bbox_head.oaie_num)*self.bbox_head.oaie_coef
    ...
    def _oamil(self, bbox_targets, bbox_results, rois, x):
        """
        Procedure:
            1. apply instance selection
            2. compute instance selection loss
        """
        loss_bbox = dict()

        # whether to perform instance selection
        if self.bbox_head.oamil_lambda > 0:
            ...
            confidence_scores, pseudo_bbox_targets, pseudo_gt_boxes = self._instance_selection(bbox_results, labels, rois, x, cur_bbox_targets)

            '''
            2. Compute instance selection loss
            '''
            loss_bbox['loss_oais'] = 0
            cls_pos_scores = self._get_instance_cls_scores(pos_labels, confidence_scores, uniq_inst, pos_indices)
            loss_bbox['loss_oais'] = (1 - cls_pos_scores) * self.bbox_head.oamil_lambda
        else:
    ...
    def _instance_selection(self, bbox_results, labels, rois, x, cur_bbox_targets):
        ...
        Procedure of instance selection:
            ...
            2. apply instance selector and output confidence score for each instance
        ...
        1. Construct object bags
        '''
        # get object bag indices from noisy gt
        pos_inds = (labels >= 0) & (labels < self.bbox_head.num_classes)
        ...
        if oais_flag:
            ...
            for index in object_bag_indices:
                ...
                # modify grount-truth according to best selected instances
        ...
        return confidence_scores, pseudo_bbox_targets, pseudo_gt_boxes
    ...
    def _get_instance_cls_scores(self, pos_labels, confidence_scores, uniq_inst, pos_indices):
        ...
        for inst in torch.unique(pos_indices):
            ...
            inst_pos_scores.append(confidence_scores[inst_inds].max().view(-1))
        ...
        for cls in torch.unique(inst_labels):
            ...
            cls_pos_scores += inst_pos_scores[cls_inds].mean()
        cls_pos_scores /= len(torch.unique(inst_labels))
        return cls_pos_scores
